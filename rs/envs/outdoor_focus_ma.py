...
from rs.envs.engine import AutoRestartManager
import copy
import numpy as np
...
class OutdoorFocusMA(EnvBase):
    ...
    def __init__(
        ...
        *,
        ...
        self.focal_low = torch.tensor(
            [[[-55.0, -111.0, -4.0] for _ in range(self.n_targets)]], device=device
        )
        self.focal_high = torch.tensor(
            [[[29.0, 120.0, 5.0] for _ in range(self.n_targets)]], device=device
        )
        ...
        rx_low = torch.ones_like(agent_pos, device=device) * (-200)
        rx_high = torch.ones_like(agent_pos, device=device) * 200
        rf_low = torch.ones_like(agent_pos, device=device) * (-200)
        rf_high = torch.ones_like(agent_pos, device=device) * 200
        ...
        self.rx_polygon_coords = [
            [(17.1273, 55.6554), (-80.5955, -9.37995), (-49.7526, -57.0043), (53.9442, 12.3049)]
    ...
    def _generate_moved_rx_positions(self, pos: np.ndarray, polygon: Polygon) -> Point:
        ...
        """
        r = 2  # radius of the circle to move the position
