...
def generate_scene(args):
    ...
    # merge tiles groups (0, 3, 6, 9, ...), (1, 4, 7, 10, ...), (2, 5, 8, 11, ...)
    tile_groups_0 = []
    for i in range(0, 48, 3):
        tile_groups_0.extend(tile_groups[i])
    tile_groups_1 = []
    for i in range(1, 48, 3):
        tile_groups_1.extend(tile_groups[i])
    tile_groups_2 = []
    for i in range(2, 48, 3):
        tile_groups_2.extend(tile_groups[i])
    tile_groups = [tile_groups_0, tile_groups_1, tile_groups_2]
    ...
    butils.save_mitsuba_xml(
        folder_dir,
        "scenee",
        [*reflectors_names, "Wall", "Floor", *obstacle_names],
        # [*reflectors_names, "Wall", "Floor", *obstacle_names, *focals_viz_names],
    )
